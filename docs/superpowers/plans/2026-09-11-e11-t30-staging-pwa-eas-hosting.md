...
- [x] Report the final SHA, checks and externally applied actions. Отдельные подтверждения получены перед EAS link/first deployment, проверкой preview variables, staging Supabase Redirect URL и browser PWA smoke; iPhone offline-приёмка остаётся E11-T31.
