...
export function shiftScheduleBlockToDate(block: ScheduleBlock, target: string, source = getDateInTimeZone(block.startsAt, block.timeZoneId)): ScheduleBlock { const start = zoneParts(new Date(block.startsAt), block.timeZoneId); const end = zoneParts(new Date(block.endsAt), block.timeZoneId); const shift = Math.round((dateOf(target).getTime() - dateOf(source).getTime()) / 86_400_000); const endDate = addDays(`${end.year}-${String(end.month).padStart(2, '0')}-${String(end.day).padStart(2, '0')}`, shift); return { ...block, startsAt: zonedDateTime(target, start, block.timeZoneId), endsAt: zonedDateTime(endDate, end, block.timeZoneId) }; }
