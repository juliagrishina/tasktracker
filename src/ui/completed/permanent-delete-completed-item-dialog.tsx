...
export function PermanentDeleteCompletedItemDialog({ error, isDeleting, itemTitle, onConfirm, onRequestClose, scope = 'item', visible }: PermanentDeleteCompletedItemDialogProps) {
  ...
  return <Modal animationType="fade" onRequestClose={onRequestClose} transparent visible={visible}><View style={styles.overlay}><View accessibilityViewIsModal style={styles.dialog}>
    <Text style={styles.title}>{deletesSeries ? 'Удалить всю серию?' : 'Удалить экземпляр?'}</Text>
    ...
    <Pressable accessibilityLabel={deletesSeries ? 'Подтвердить удаление всей серии' : 'Подтвердить удаление экземпляра'} accessibilityRole="button" disabled={isDeleting} onPress={onConfirm} style={[styles.action, styles.deleteAction, isDeleting && styles.disabled]}><Text style={styles.deleteText}>{isDeleting ? 'Удаляем…' : deletesSeries ? 'Удалить всю серию' : 'Удалить экземпляр'}</Text></Pressable>
    <Pressable accessibilityLabel={deletesSeries ? 'Отменить удаление всей серии' : 'Отменить удаление экземпляра'} accessibilityRole="button" disabled={isDeleting} onPress={onRequestClose} style={[styles.cancel, isDeleting && styles.disabled]}><Text style={styles.cancelText}>Отмена</Text></Pressable>
  </View></View></Modal>;
}
