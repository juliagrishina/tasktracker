...
export function PermanentDeleteCompletedItemDialog({ error, isDeleting, itemTitle, onConfirm, onRequestClose, visible }: PermanentDeleteCompletedItemDialogProps) {
  return <Modal animationType="fade" onRequestClose={onRequestClose} transparent visible={visible}><View style={styles.overlay}><View accessibilityViewIsModal style={styles.dialog}>
    <Text style={styles.title}>Удалить безвозвратно?</Text>
    <Text style={styles.description}>«{itemTitle}» исчезнет из архива без возможности восстановления.</Text>
    {error === null ? null : <Text accessibilityLiveRegion="polite" style={styles.error}>{error}</Text>}
    <Pressable accessibilityLabel="Подтвердить безвозвратное удаление" accessibilityRole="button" disabled={isDeleting} onPress={onConfirm} style={[styles.action, styles.deleteAction, isDeleting && styles.disabled]}><Text style={styles.deleteText}>{isDeleting ? 'Удаляем…' : 'Удалить безвозвратно'}</Text></Pressable>
    <Pressable accessibilityLabel="Отменить безвозвратное удаление" accessibilityRole="button" disabled={isDeleting} onPress={onRequestClose} style={[styles.cancel, isDeleting && styles.disabled]}><Text style={styles.cancelText}>Отмена</Text></Pressable>
  </View></View></Modal>;
}
